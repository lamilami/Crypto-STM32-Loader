...
stm32_t* stm32_init(const serial_t *serial, const char init) {
	...
	if (init) {
		...
		if (stm32_read_byte(stm) != STM32_ACK) {
			stm32_close(stm);
			printf("Failed to get init ACK from device\n");
			return NULL;
		}
	}
	...
	if (stm->cmd->get == STM32_CMD_ERR
		...
		printf("Error: bootloader did not returned correct "
	...
	if (len > 0) {
		...
		while (len-- > 0)
			printf( " %02x", stm32_read_byte(stm));
		printf("\n");
	}
	if (stm32_read_byte(stm) != STM32_ACK) {
		stm32_close(stm);
	...
	if (!stm->dev->id) {
		printf( "Unknown/unsupported device (Device ID: 0x%03x)\n", stm->pid);
		stm32_close(stm);
		return NULL;
	}
...
char stm32_read_memory(const stm32_t *stm, uint32_t address, uint8_t data[], unsigned int len) {
	...
	if (stm->cmd->rm == STM32_CMD_ERR) {
		printf("Error: READ command not implemented in bootloader.\n");
		return 0;
	}
...
char stm32_write_memory(const stm32_t *stm, uint32_t address, const uint8_t data[], unsigned int len) {
	...
	if (stm->cmd->wm == STM32_CMD_ERR) {
		printf("Error: WRITE command not implemented in bootloader.\n");
		return 0;
	}
...
char stm32_wunprot_memory(const stm32_t *stm) {
	if (stm->cmd->uw == STM32_CMD_ERR) {
		printf("Error: WRITE UNPROTECT command not implemented in bootloader.\n");
		return 0;
	}
...
char stm32_runprot_memory  (const stm32_t *stm) {
	if (stm->cmd->ur == STM32_CMD_ERR) {
		printf("Error: READ UNPROTECT command not implemented in bootloader.\n");
		return 0;
	}
...
char stm32_readprot_memory(const stm32_t *stm) {
	if (stm->cmd->rp == STM32_CMD_ERR) {
		printf("Error: READ PROTECT command not implemented in bootloader.\n");
		return 0;
	}
...
char stm32_erase_memory(const stm32_t *stm, uint8_t spage, uint8_t pages) {
	...
	if (stm->cmd->er == STM32_CMD_ERR) {
		printf("Error: ERASE command not implemented in bootloader.\n");
		return 0;
	}
...
char stm32_go(const stm32_t *stm, uint32_t address) {
	...
	cs      = stm32_gen_cs(address);
        ...
        if (stm->cmd->go == STM32_CMD_ERR) {
               printf("Error: GO command not implemented in bootloader.\n");
                return 0;
        }
