...
////////////////////////////////////////////////////////// INIT MMC and FAT 
  if(ini_openread("loader.bin",&f))
             		fr = f_stat("loader.bin", &fno);
	else return 20;
  //printf("r1= %u\n",res);		
 ...
 if(RxC==ACK){
                 ...
                 UartPutChar(address.bytes[3]);
                 UartPutChar(address.bytes[2]);  
                 UartPutChar(address.bytes[1]);
	               UartPutChar(address.bytes[0]);
	 
	  
     checksum = address.bytes[0] ^ address.bytes[1] ^ address.bytes[2] ^ address.bytes[3]; // this is not a checksum
           
                UartPutChar(checksum);
...
////////////////////////////////////////send byte count for Write Memory command
    ...
    if(ini_read(ReadBuf,buffer_size,&f))
		{         
