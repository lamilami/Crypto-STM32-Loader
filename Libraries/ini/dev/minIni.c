...
#if defined INI_ANSIONLY //!defined __T || 
  #include <ctype.h>
  #include <string.h>
  #include <stdlib.h>
//  #define TCHAR     char
  ...
  #define _tcsnicmp strncmp              //strnicmp
                        ...
                        INI_FILEPOS *mark)
{
  ...
  if (len > 0 || idxSection >= 0) {
    ...
    do {
        ...
        return 0;
      sp1 = skipleading(LocalBuffer);
      ep = _tcschr(sp1, ']');
    } while (*sp1 != '[' || ep == NULL || (((int)(ep-sp1-1) != len || _tcsnicmp(sp1+1,Section,len) != 0) && ++idx != idxSection));
    if (idxSection >= 0) {
      if (idx == idxSection) {
        assert(ep != NULL);
        assert(*ep == ']');
        *ep = '\0';
        save_strncpy(Buffer, sp1 + 1, BufferSize, QUOTE_NONE);
        return 1;
      } /* if */
  ...
  do {
    ...
    if (!ini_read(LocalBuffer,INI_BUFFERSIZE,fp) || *(sp1 = skipleading(LocalBuffer)) == '[')
      return 0;
    sp1 = skipleading(LocalBuffer);
    ep = _tcschr(sp1, '=');  /* Parse out the equal sign */
    if (ep == NULL)
      ep = _tcschr(sp1, ':');
  } while (*sp1 == ';' || *sp1 == '#' || ep == NULL
           || ((len == 0 || (int)(skiptrailing(ep,sp1)-sp1) != len || _tcsnicmp(sp1,Key,len) != 0) && ++idx != idxKey));
  if (idxKey >= 0) {
    if (idx == idxKey) {
      assert(ep != NULL);
      assert(*ep == '=' || *ep == ':');
      *ep = '\0';
      striptrailing(sp1);
      save_strncpy(Buffer, sp1, BufferSize, QUOTE_NONE);
      return 1;
    } /* if */
  ...
  assert(ep != NULL);
  assert(*ep == '=' || *ep == ':');
  sp1 = skipleading(ep + 1);
  sp1 = cleanstring(sp1, &quotes);  /* Remove a trailing comment */
  save_strncpy(Buffer, sp1, BufferSize, quotes);
  return 1;
}
